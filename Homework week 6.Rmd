...
g= ggplot(data = d, aes(x=log(Brain_Size_Species_Mean), y=MaxLongevity_m)) + geom_point() + geom_smooth(method = "lm", formula = y ~ x) + geom_text(aes(x=.5, y=.7,label= "MaxLongevity_m ~ Brain_Size_Species_Mean"))
g
```
...
g= ggplot(data = d, aes(x=Brain_Size_Species_Mean, y=MaxLongevity_m)) + geom_point() + geom_smooth(method = "lm", formula = y~x) + geom_text(aes(x=.1, y=.2,label= "MaxLongevity_m ~ Brain_Size_Species_Mean"))
g

#FIX LABEL THINGY FOR TEXT OF MODEL
```
